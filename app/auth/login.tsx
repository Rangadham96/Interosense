...
export default function LoginScreen() {
  ...
  const { login, loginWithSocial, refreshUser } = useAuth();
  ...
  const initGSI = React.useCallback(() => {
    ...
    const container = document.getElementById('__gsi_login');
    if (container) {
  ...
  const handleGoogleSignIn = async () => {
    setError('');
    if (Platform.OS === 'web') {
      const btn = document.querySelector('#__gsi_login [role="button"], #__gsi_login button') as HTMLElement | null;
        ...
        <View style={styles.content}>
          ...
          <View style={styles.formCard}>
            ...
            {Platform.OS === 'web' && GOOGLE_CLIENT_ID ? (
              <View nativeID="__gsi_login" style={{ position: 'absolute', top: -1000, left: -1000, width: 50, height: 50 }} />
            ) : null}
